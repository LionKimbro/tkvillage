...
def route_effect(effect, source_id=None):
    append_log("effect_log", {"at": time.time(), "source_id": source_id, "effect": effect})
    ...
    if effect_type == "REQUEST_PROJECT":
        ...
        if window_id in app["windows"]:
            app["windows"][window_id]["needs_project"] = True
        return None
    if effect_type == "LOG":
        append_log("runtime_log", {"at": time.time(), "source_id": source_id, "message": effect.get("message")})
        return None
    if effect_type == "SHOW_MESSAGE_BOX":
        return messagebox.showinfo(effect.get("title", "TkVillage"), effect.get("message", ""))
    if app["strict_effects"]:
        raise ValueError(f"Unknown effect type: {effect_type}")
    append_log("runtime_log", {"at": time.time(), "source_id": source_id, "message": f"Unknown effect: {effect_type}"})
    return None
