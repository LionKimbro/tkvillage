...
def register_debug_windows():
    ...
    if DEBUG_DASHBOARD_KIND in app["window_kinds"]:
        return
...
def install_debug_shortcut():
    root = app["root"]
    root.bind_all("<Control-Shift-D>", lambda _event: toggle_debug())


def toggle_debug():
    app["debug"]["enabled"] = not app["debug"]["enabled"]
...
def get_debug_snapshot(selected_window_id=None):
    snapshot = {
        "app": {
            ...
            "debug_enabled": app["debug"]["enabled"],
        ...
        "window_kinds": {
            ...
            for name, record in app["window_kinds"].items()
        },
        "active_windows": {
            window_id: summarize_window(record) for window_id, record in app["windows"].items()
        },
        "recent_events": list(app["event_log"]),
        "recent_effects": list(app["effect_log"]),
        "recent_messages": list(app["message_log"]),
        "runtime_log": list(app["runtime_log"]),
    }
    if selected_window_id is not None and selected_window_id in app["windows"]:
        snapshot["selected_window_state"] = app["windows"][selected_window_id]["state"]
    app["debug"]["last_snapshot"] = snapshot
    return snapshot
